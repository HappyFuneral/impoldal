...
const LibrarySong = ({ song, setCurrentSong, audioRef, isPlaying, songs, setSongs }) => {
	...
	return (
		<LibrarySongContainer className="text-gray-300 bg-gray-700" onClick={songSelectHandler} isActive={song.active}>
			<Img src={song.cover} alt={song.name}></Img>
			<LibrarySongDescription>
				<H1 className="text-gray-300" >{song.name}</H1>
				<H2 className="text-gray-300">{song.artist}</H2>
			</LibrarySongDescription>
...
const LibrarySongContainer = styled.div`
	...
	background-color: ${(p) => (p.isActive ? "gray" : "")};
	&:hover {
